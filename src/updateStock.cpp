...
void updateStock() {
	...
	printf("==========물품이 입고된 경우 재고를 추가하는 메뉴입니다==========\n");
	...
	printf("재고를 추가할 물품의 id 를 입력하세요 : ");
	scanf("%d", &id);

	printf("입고된 물품의 개수를 입력하세요 : ");
	scanf("%d", &num);


	if (SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt) == SQL_SUCCESS) {
		sprintf((char*)query, "UPDATE PRODUCT SET STOCK = STOCK + %d WHERE STOID = ( SELECT STOID FROM STORE WHERE STONAME = '%s') AND PRODID = %d",num,stoname,id);
